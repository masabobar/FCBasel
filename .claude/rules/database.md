...
## Test Re-Validation on Schema / Entity Change

**MANDATORY: when you change a model in `schema.prisma`, add a migration, or modify an entity's service/repository, find and re-run ALL existing tests that exercise that entity — not just new tests.** A schema change (renamed column, changed type, new constraint, removed field) silently breaks any test asserting the old shape; running only your new tests hides the regression.

Find related tests by entity name (same grep mechanism as `.claude/rules/api-versioning.md` §5.3 uses for endpoints):

```bash
grep -rln "<ModelName>" tests/          # e.g. "Order" — model + its services
# also try table-name / field-name variants if the entity is referenced by raw name
```

Re-run every match. A failing related test is a signal — either it encoded a real expectation the schema change broke, or the migration is breaking and needs a deprecation path. Fix it correctly; never mirror the new shape into the test just to make it green.

`/run-tests entity <ModelName>` runs exactly this scoped set. This re-validation is part of the mandatory final test step enforced by `/execute-work` (per `.claude/rules/testing.md` → Related-Test Re-Validation).

---

**Related:**
- `.claude/rules/testing.md` — Related-Test Re-Validation (the shared mechanism for API / DB / enum changes)
- `.claude/rules/enums-and-constants.md` — enum changes (often correlated with schema changes)
- Main `CLAUDE.md` for overall development workflow.

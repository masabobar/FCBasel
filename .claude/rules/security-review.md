...
| Any new/changed HTTP handler or route | A01/A05 | Default-deny auth (`requireAuth`/`requireRole` unless explicitly public), Zod validation of body/params/query, security headers present (`security-and-auth.md` §3,§4,§5) |
...
| A04 | Insecure Design | §1 file-upload + threat-think during plan mode; abuse cases in story acceptance criteria |
| A05 | Security Misconfiguration | §1 security headers (CSP/HSTS/X-Frame/X-Content-Type), CORS allowlist (`security-and-auth.md` §5) |
...
- `commands/modules/execute-work-quality-gates.md` — where the triage gate is enforced

...
2. **Open a backend story or bug** that closes the gap. If the project has a bug roadmap, file it via `/add-bug` (routes to `.project-management/output/bugs/<severity>.md`) with category `API contract gap`. If a backlog phase is appropriate, add a backend story via `/add-scope`.
...
- `commands/execute-work.md` — plan-mode hook that enforces this gate

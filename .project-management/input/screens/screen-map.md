...
**Last Refreshed By `/screen-map`:** *(hand-updated 2026-09-10 after US-037 — Hero 2 now runs END TO END: its follow-up beat ranks the four declining fixtures behind the shared gold seam, with a derived -CHF 400k badge and an attendance note. Earlier note, after US-036 — Hero 2's primary answer is BUILT: the fixture chart, the derived totals tile and the full-width monthly chart, with **each chart carrying its own scope label**. Still no API: one more static read in the same root loader. Earlier note, 2026-09-09 after US-031 — the **Thinking** state is now BUILT: every answer waits behind a fixed staged delay, and the transient panel that fills it is the first of the three. Still no API: the delay is a `setTimeout`, not a request)*
**Status:** Approved
...
## 4. Drift Report (hand-checked 2026-09-10, after US-037)

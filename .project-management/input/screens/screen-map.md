...
| **Stories** | US-012, US-013, US-014, US-015, US-016, US-028 to US-039, US-049 |
...
**Language:** the app bar carries an **EN / DE toggle** (US-049). One press re-renders the whole
screen — chrome, prepared questions, panels and every answer — from the two dictionaries in
`app/lib/i18n/locales/`. English is the default; the selection is memory-only, so a reload returns
to it.
...
## 4. Drift Report (hand-checked 2026-09-10, after US-044)

- **Stories referencing screens not in this map:** *(none)*
- **Routes added by US-044:** *(none)*, and no region, state or content was added either. It is a
  measurement story: `tests/e2e/brand-fidelity.spec.ts` + `tests/e2e/support/brand.ts` walk the
  SERVED page at four moments (baseline, mid-beat, full script, fallback) and classify every
  painted colour against `app/lib/tokens.ts`. The one screen change is a SIGN on Hero 3's driver
  total badge (`CHF 410k` -> `+CHF 410k`), noted in SCREEN-001's Insight sections row above; the
  bar labels beside it stay unsigned, because a row is an amount and the badge is a movement.

...
**Current Phase:** Phase 2b - Chart & Tile Component Library *(4/11 stories complete)* · Phase 2a partial (4/5, US-016 still deferred) · Phases 1a + 1b complete
...
|--------|-------|--------|--------|
| **Overall Progress** | 40% | 100% | 🟢 On Track |
| **Phase 1a** | 100% — Completed | 100% | 🟢 Done |
| **Phase 1b** | 100% — Completed | 100% | 🟢 Done |
| **Stories Completed** | 19/45 | 45 | 🟢 On Track |
| **Story Points Done** | 46/116 | 116 | 🟢 On Track |
...
**Stories Completed Today:** 19
**Currently Working On:** US-026 — Segmented period filter control (2 pts)
**Story Points Completed Today:** 46
...
- ✅ **US-003 — Design token set (3 pts)** — one token set published as Tailwind v4 `@theme static`
  ...
  discipline is encoded in the token names, not just documented.
- ✅ **US-004 — Self-hosted FCB crest (1 pt)** — the club's `logo.webp` is actually a PNG; verified by
  ...
  metadata stripped. Served from `public/`; build and server contain zero `fcb.ch` references.
- ✅ **US-005 — Tile card anatomy (2 pts)** — one `Card` shell (plus its narrative caption strip) that
  ...
  no hex reaches a tile. Entrance hooks only — US-006 owns the motion, and there is no gold ring.
- ✅ **US-006 — Tile-insertion motion & reduced-motion support (3 pts)** — the four reveal keyframes
  defined once, timed from motion tokens, wired to the card's `isNew`/`delayMs` hooks. Fade-and-rise
  only. Reduced motion *collapses* animations to a ~1ms final frame rather than removing them.
...
- ✅ **US-008 — Hero 1 dataset (2 pts)** — season-to-date merchandising: three kits, the sponsor badge
  split and the top five printed names, across all four periods. Kit revenue (units x CHF 99), the
  58% Home share and the exactly-8% badge share are *derived*, never stored. `badgeSegments` corrects
  its rounding remainder into the first segment so the donut adds up to the number printed inside it.
...
- ✅ **US-009 — Hero 2 dataset (2 pts)** — matchday ticket revenue year on year: eight home fixtures
  ...
  sit at *different scopes on purpose*, so each carries its own `scopeLabel` and a test proves they
  differ. Totals, the -0.6% and the -CHF 400k badge are all *derived* from the fixture pairs.
...
- ✅ **US-011 — Formatters & cross-hero reconciliation (2 pts)** — the shared display layer
  ...
  *before* the unit (`-CHF 400k`); millions render bare under the "figures in CHF millions" subtitle.
  `Intl.NumberFormat("en-CH")` per the Reference Guide, so thousands group with the Swiss U+2019 mark
  — pinned as a constant and made ICU-independent, proved by stubbing `Intl` to `en-US` and `de-DE`.
  `oneDecimal` is *imported* from `derive.ts`, so there is one rounding rule, and variance carries
  meaning through sign plus a `VarianceDirection` enum rather than colour. The reconciliation suite
  asserts relationships, not constants — every stored key set, every split against its total, the
  intended cross-hero inequality, and every number in all six narratives. **No drift found.**
...
- ✅ **US-017 — KPI tile & variance chip (2 pts)** — the first component built ON the motion hooks,
  ...
  US-016's navy band composes `KpiFigure onDark`, forcing the light chip. **The US-012
  `tailwind-merge` trap is closed at the root:** `app/lib/cn.ts` declares the named type scale as a
  font-size group, so a size and a colour can share an element everywhere from here on. 78 tests.
...
- ✅ **US-013 — Baseline dashboard, four pre-existing tiles (3 pts)** — **the canvas stops being
  ...
  Partner plates carry the partner's **own** brand colour from the data (a test asserts no hex and no
  FCB token in the file, because a plate in club red is wrong to a sponsor in the room).
  **Reset's baseline seam is closed as US-015 described:** the tiles are static route chrome outside
  the session list, so no question can remove them; a test drives insert → Reset and compares the
  canvas `innerHTML` to its load state. **First real-Chrome pass for US-017, US-021 and US-027**: no
  horizontal scroll at 1920×1080 (nor 1440/1280/834/390), 54 distinct KPI strings and 43 distinct bar
  widths per frame, and two values only under `prefers-reduced-motion`. 103 new tests.
...
|-------|--------|----------|
| US-026: Segmented period filter control | 🔄 Next | The last piece US-016 waits on |
| US-016 (Phase 2a) | ⏸️ Deferred | Awaits US-026 only |
...
| Phase 2b: Component Library | 🔄 In Progress | 4/11 | 11/29 | 38% |
...
- **[Phase 2a Plan](../phases/phase-2a.md)** - Partial (4/5); US-016 still deferred to US-026
...
**Last Auto-Update:** US-025 completed at 2026-09-09 — **Phase 2b at 4/11 · 11/29 pts**. One line chart serves both the navy hero band and Hero 2's month-by-month comparison: series count is a prop, `area` / `dash` is per series, colour is a token name, and the stroke-draw entrance replays on a re-key while rendering fully drawn in the first render under reduced motion. Next is US-026, the segmented period filter — the last piece US-016 is waiting on

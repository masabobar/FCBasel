...
**Setup (standalone):** Copy `commands/` + `rules/` + `hooks/` into `.claude/`, `.project-management/` into project → Customize `input/*.md` → Run `/init-project`

**Setup (plugin):** `/plugin marketplace add HolycodeGroup/claude_repo` → `/plugin install holycode-pm@holycode` → `/holycode-pm:setup` (bootstraps `rules/` → `.claude/rules/` + templates) → `/holycode-pm:init-project`. Distributed privately via git access (GitHub org membership), not a public listing or email gate. Commands are namespaced `/holycode-pm:*`. See root `README.md` → "Install as a Private Plugin".
...
3. **Specialized Rules** (`.claude/rules/`) - load by category; mandatory always, conditional when the trigger applies
   ...
   **Always (every story / bug):**
   ...
   - `security-review.md` - **Universal security triage + OWASP Top 10 — EVERY story/bug (MANDATORY)**; routes to `security-and-auth.md` when risk triggers fire
   ...
   **Conditional — load when the trigger fires:**
   ...
   - `cost-and-model-optimization.md` - When dispatching sub-agents or authoring a scan / audit / status command: mechanical-vs-reasoning model tiering (Haiku for command-runs, Opus for code/judgement), prompt-cache the rules prefix, parallel read-only fan-out
...
**PM:** `/adopt-project` (onboard existing code), `/init-project` (greenfield), `/project-status` (live view: `output/progress/DASHBOARD.md`)
...
**Open questions:** `/process-client-docs` runs an interactive Q&A gate (STEP 5) — skipped questions persist to `input/open-questions.md`; resume with `/resolve-questions [--priority Px | Q-NNN]`. Pattern lives in `commands/modules/interactive-clarifications.md` for future commands.
...
**Version:** 3.7.3 (Progress Visibility Fixes)
**Updated:** 2026-06-11
